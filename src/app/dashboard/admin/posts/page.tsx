          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <FiMessageSquare className="text-purple-500" /> Community Moderation
              </h1>
              <p className="text-sm text-gray-500 mt-1">Moderate community posts, images, and user content.</p>
            </div>
            {pagination && (
              <span className="text-sm font-medium bg-white dark:bg-gray-800 px-3 py-1.5 rounded-full text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700">
                Total Posts: <strong className="text-gray-900 dark:text-white">{pagination.total}</strong>
              </span>
            )}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-t-2xl border border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="relative w-full sm:w-96">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-900 dark:text-white focus:outline-none focus:border-purple-500 transition-colors"
            />
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-gray-500">Loading posts...</td>
                  </tr>
                ) : filteredPosts.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-gray-500">No posts found.</td>
                  </tr>
                ) : (
                  filteredPosts.map((p) => (
                      ...
                      <td className="px-6 py-4">
                        <div className="relative w-12 h-12 rounded-lg overflow-hidden bg-gray-100">
                          {p.imageUrl && (p.imageUrl.startsWith("http") || p.imageUrl.startsWith("/")) && (
                            <img src={p.imageUrl} alt="Post media" className="object-cover w-full h-full" />
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="font-medium text-gray-900 dark:text-white">{p.author?.name || "Unknown"}</div>
                        <div className="text-xs text-gray-500">{p.author?.email || ""}</div>
                      </td>
                      <td className="px-6 py-4 text-gray-600 dark:text-gray-300">
                        <p className="line-clamp-2 max-w-xs">{p.caption}</p>
                      </td>
                      <td className="px-6 py-4 text-gray-600 dark:text-gray-300">
